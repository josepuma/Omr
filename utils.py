...
class Respuesta:
    ...
    def getAnswer(self, type, debug = False):
        if type == '2':
            return 'N'
        arr = []
        minimo = 100 #El valor 80 es un aproximado, generalmente los numeros altos estan cerca de este rango
        maximo = 0
        for col in self.cols:
        ...
        for fila in arr:
            if (intermedio + 10) > fila :
                ...
                if nMarcadas > 1 :
                    result = 'NULO'
                if nMarcadas > 2 and maximo < 80 :
                    result = 'VACIO'      
                   
            # archivo = open('avg.txt','a')
            # archivo.write('avg_'+ str(self.nPregunta) + '__' + str(fila)+ ' resultado : '+ str(result))
            # archivo.write('     minimo: '+ str(minimo) + '     Máximo:  ' + str(maximo) + 'Máximo:  ' + str(intermedio) +'        marcadas: '+ str(nMarcadas) +'         fila:'+ str(fila) +'\n')
